...
| Patch sampling matching v2 design: `gee_s1s2/sampling.py` exposes random-origin sampling with the v2 `random_patches` strategy and min-spacing rule. `export.py` submits one ``Export.image.toCloudStorage`` task per origin (256 m × 256 m sub-region clip), with rate-limited chunked submission (`max_concurrent_tasks`, `task_submit_chunk_size` in the export config) and resume state in `.gee_s1s2_harvest_state.json` so an interrupted harvest doesn't re-submit work in flight. For small AOIs the sampler logs `random_patches: only N/M sampled` when the budget exceeds AOI capacity. |
